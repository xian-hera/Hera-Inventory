...
// Create BOX PO (see claude/BOX_PO_FEATURE_SPEC.md section 2). Single-step
// page (simplified 2026-09-25, Hera): Supplier + optional Date sit directly
// in the toolbar, left of "Add line" — no separate Card1/Confirm step
// anymore, and no Total BOXES field at all (total_boxes is now computed
// server-side from the line items' BOX qty on Create; nothing here collects
// or persists it).
function BuyerBoxPOCreate() {
  ...
  const handleCreate = async () => {
    if (!supplierId) { setCreateError('Supplier is required.'); return; }
        ...
        <Layout.Section>
          <BlockStack gap="400">
            ...
            <InlineStack gap="200" wrap blockAlign="end">
              {(suppliersLoading || locationsLoading) ? (
