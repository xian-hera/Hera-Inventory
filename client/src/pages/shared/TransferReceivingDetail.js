...
// Shared Receiving-side detail page — handles both In transit (read-only,
// "Delivered" button) and Receiving (full counting page modeled on
// ManagerPOReceivingDetail.js: scanner, count popup, All/Uncounted/Off-qty
// filter pills, Wig Number column, notes).
//
// 2026-09-15: generalized from what used to be Manager-only
// (ManagerTransferReceivingDetail.js) so Warehouse's new "Receiving to HQ"
// card (改动二) can reuse the exact same page — Hera confirmed the structure
// is identical, Wig Number included. The one behavioral difference is the
// `role` prop: warehouse gets a "Submit Without Counting" button whenever at
// least one item is still uncounted (→ status 'not_counted', Buyer finishes
// the rest); manager keeps the original all-or-nothing Submit gate. Also
// adds: hold banner/button-disable (改动六 — this page is never rendered for
...
function TransferReceivingDetail({ role, backPath }) {
  ...
  const wigContextParam = role === 'warehouse' ? '&wigContext=receiving-hq' : '';

  const fetchTransfer = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const res = await fetch(`/api/transfers/${transferId}?role=${role}${wigContextParam}`);
  ...
  const openPopup = (item) => {
    if (held || item.edit_state === 'removed') return;
    setPopupItem(item);
    setCountInput('');
    setCountError('');
  };
  ...
  const handleDelivered = async () => {
    ...
    setError('');
    try {
      await postWithConflictCheck(`/api/transfers/${transferId}/delivered`, {});
      await fetchTransfer();
    } catch (e) {
  ...
  // (to-location) qty column, per Hera's column spec.
  ...
  const handleSubmit = async (force) => {
    ...
    setSubmitError('');
    try {
      await postWithConflictCheck(`/api/transfers/${transferId}/submit-count`, { force });
      navigate(backPath);
    } catch (e) {
      if (e.message !== '__conflict__') setSubmitError(e.message);
  ...
  const countedCount = activeItems.filter(i => i.counted_confirmed).length;
  const allCounted = totalCount > 0 && countedCount === totalCount;
  const notCountedCount = totalCount - countedCount;
  const offQtyCount = activeItems.filter(i => i.counted_confirmed && Number(i.received_quantity) !== Number(i.quantity)).length;

  // 改动二 (Warehouse only): when incomplete, the main button is "Submit
  // Without Counting" instead of being disabled — it advances to
  // 'not_counted' and leaves the remaining rows for Buyer to finish. Manager
  // keeps the original all-or-nothing gate.
  const canForceSubmit = role === 'warehouse';
  const submitLabel = allCounted ? 'Submit' : (canForceSubmit ? 'Submit Without Counting' : 'Submit');
  const submitDisabled = held || (!allCounted && !canForceSubmit);
  ...
  // Off-qty rows and Buyer-edited rows always pinned to top — alphabetical
  // by name otherwise, and within each group too.
  ...
  const filteredItems = sortedItems.filter(item => {
    ...
    if (itemFilter === 'not_counted') return !item.counted_confirmed && item.edit_state !== 'removed';
              ...
              <Card>
                <BlockStack gap="300">
                  <InlineStack gap="200" wrap align="end">
                    ...
                    <Button variant="primary" onClick={() => setShowSubmitConfirm(true)} disabled={submitDisabled}>
                      {submitLabel}
                    </Button>
                  </InlineStack>
                      ...
                      {filteredItems.map(item => {
                        ...
                        return (
                          <tr
                            ...
                            style={{ borderBottom: '1px solid #f1f1f1', cursor: removed || held ? 'default' : 'pointer', ...rowStyle(item) }}
                          >
        ...
        {showSubmitConfirm && (
          <div style={{
            ...
            <div style={{ background: 'white', borderRadius: '12px', padding: '24px', width: '100%', maxWidth: '360px' }}>
              <BlockStack gap="300">
                <Text variant="headingMd" fontWeight="bold">{allCounted ? 'Submit this count?' : 'Submit without counting everything?'}</Text>
                <Text variant="bodyMd" tone="subdued">
                  {allCounted
                    ? 'This transfer will move to Counted and be removed from your Receiving list.'
                    : `${notCountedCount} item(s) are still uncounted. This transfer will move to Not counted and be removed from your Receiving list — the buyer will finish counting and commit it.`}
                </Text>
                <InlineStack gap="200" align="center">
                  <button
                    onClick={() => handleSubmit(!allCounted)}
                    disabled={submitting}
                    style={{
...
export default TransferReceivingDetail;
